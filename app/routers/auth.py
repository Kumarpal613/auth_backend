...
def login(response: Response, db: db_dep, form_data: form_dep):
    ...
    if not user or not security.verify_password(
        password=form_data.password, hashed=user.password
    ):
        raise auth_exception

    access_token, refresh_token = auth_service.create_refresh_and_access_tokens(
        db, user.id
    )
...
def refresh(request: Request, response: Response, db: db_dep):
    ...
    access_token, new_refresh = auth_service.create_refresh_and_access_tokens(
        db, user.id
    )
...
def request_password_recovery(request: auth.ForgotPasswordReq, db: db_dep):
    ...
    return {
        "message": "If an account with that email exists, a password recovery email has been sent.",
        "recovery_token": response_data["recovery_token"],
    }
...
def resend_recovery_otp(
    db: db_dep, auth: HTTPAuthorizationCredentials = Depends(security_scheme)
):
    token = auth.credentials
